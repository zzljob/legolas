...
public class RequestBuilder {
	...
	/**
	 * 处理Listener<br/>
	 * 起始监听器在定义的时候一定要定义成Request.RequestListener<br/>
	 * 成功监听器在定义的时候一定要定义成Response.Listener<?><br/>
	 * 失败监听器在定义的时候一定要定义成Response.ErrorListener<?>
	 * @param p
	 * @param obj
	 * @return
	 */
	private boolean setListener(Parameter p, Object obj) {
		...
		Class<?> clazz = p.clazz;
		if (clazz.equals(Request.RequestListener.class)) {
			requestListener = (Request.RequestListener) obj;
		}
		if (clazz.equals(Response.Listener.class)) {
			responseListener = (Response.Listener<?>) obj;
		}
		if (clazz.equals(Response.ErrorListener.class)) {
			errorListener = (Response.ErrorListener) obj;
		}
		return true;
	}
	...
	public Request build() throws Exception {
		...
		r.setRequestListener(requestListener);
		r.setResponseListener(responseListener);
		r.setErrorListener(errorListener);
		r.setResult(request.getResponseType());
		log.v(String.format("RequestListener:%s, ResponseListener:%s, ErrorListener:%s", requestListener, responseListener, errorListener));
