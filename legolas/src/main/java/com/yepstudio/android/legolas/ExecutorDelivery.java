...
import com.yepstudio.android.legolas.http.Request.RequestListener;
    ...
    private class ResponseDeliveryRunnable implements Runnable {
        ...
        public void run() {
			if (mResult == null && mError == null) {
				RequestListener requestListener = mRequest.getRequestListener();
				if (requestListener != null) {
					requestListener.onRequest();
				}
			} else if (mResult != null && mError == null) {
				Response.Listener listener = mRequest.getResponseListener();
				if (listener != null) {
					listener.onResponse(mResult);
				}
			} else if (mResult == null && mError != null) {
				Response.ErrorListener errorListener = mRequest.getErrorListener();
				if (errorListener != null) {
					errorListener.onErrorResponse(mError);
				}
			}
